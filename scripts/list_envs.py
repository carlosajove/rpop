...
All the environments are registered in the `r1_lab` extension. They start
...
with contextlib.suppress(ImportError):
    import r1_lab.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~r1_lab.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from r1_lab.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `r1_lab` extension."""
    ...
    if args_cli.show_presets:
        from r1_lab.utils.preset_cli import enumerate_task_presets
